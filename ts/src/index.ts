  ...
  try {
    if (isSystemIntent(chatRequest)) {
      ...
      if ("trigger_message" in result) {
        ...
        const contentText = resObj.message ? resObj.message : JSON.stringify(result);
        res.write(serializeEvent({ type: "system_message", content: contentText }));
