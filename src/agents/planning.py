...
PLANNING_SYSTEM_PROMPT = """Você é o Planning Agent da Cloudinha, assistente educacional do Nubo Conecta.
...
Opções: search_opportunities, get_student_profile, lookup_cep, get_match_results, search_institutions
...
async def run_planning_agent(user_message: str, lean_context: str) -> StructuredPlan:
    ...
    raw = await _call_planning(client, prompt)
    try:
    ...
    try:
        raw_retry = await _call_planning(client, corrective_prompt)
...
async def _call_planning(client: genai.Client, prompt: str) -> str:
